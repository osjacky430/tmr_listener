...
class TMRobotTCP {
 ...
 public:
  ...
  explicit TMRobotTCP(Callback const &t_cb, unsigned short const t_port,
                      std::string const &t_ip_addr = DEFAULT_IP_ADDRESS) noexcept
    : robot_address_{boost::asio::ip::address::from_string(t_ip_addr)}, tm_robot_{robot_address_, t_port}, cb_{t_cb} {}

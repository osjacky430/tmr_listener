...
ParseRule<TMSCTTag::DataFrame::FunctionCall>& TMSCTTag::DataFrame::FunctionCall::parsing_rule() noexcept {
  ...
  static ParseRule<TMSCTTag::DataFrame::FunctionCall> rule = function_name >> "(" >> arguments >> ")";
  return rule;
}

ParseRule<TMSCTTag::DataFrame::VariableDecl>& TMSCTTag::DataFrame::VariableDecl::parsing_rule() noexcept {
  ...
  static ParseRule<TMSCTTag::DataFrame::VariableDecl> rule = type_rule >> name_rule >> '=' >> val_rule;
  return rule;
}

ParseRule<TMSCTTag::DataFrame::ScriptResult>& TMSCTTag::DataFrame::ScriptResult::parsing_rule() noexcept {
...
ParseRule<TMSCTTag::DataFrame>& TMSCTTag::DataFrame::parsing_rule() {
  ...
  static ParseRule<DataFrame> rule = id_rule >> "," >> data_rule;
  return rule;
}
...
ParseRule<TMSTATag::DataFrame>& TMSTATag::DataFrame::parsing_rule() {
  ...
  static ParseRule<SubcmdDataMsg> rule_90_99_ = int_ >> "," >> lexeme[+(char_ - ",*")];
...
ParseRule<CPERRTag::DataFrame>& CPERRTag::DataFrame::parsing_rule() noexcept {
  ...
  static ParseRule<DataFrame> response_parser     = err_code_rule - ",*";
...
BOOST_FUSION_ADAPT_STRUCT(TMSCTTag::DataFrame::FunctionCall, name_, args_)
BOOST_FUSION_ADAPT_STRUCT(TMSCTTag::DataFrame::VariableDecl, type_, name_, val_)
BOOST_FUSION_ADAPT_STRUCT(TMSCTTag::DataFrame::ScriptResult, result_, abnormal_lines_)